...
| `MISSING *.sh text eol=lf` / shell EditorConfig | Shell scripts may pick CRLF or odd indent | Restore `.gitattributes` `*.sh text eol=lf` and the `[*.{sh,bash}]` EditorConfig section |
| `MISSING cancel-in-progress: true` on CI | Superseded PR runs pile up | Restore `cancel-in-progress: true` under CI concurrency |
| `MISSING cancel-in-progress: false` on release | Tag release jobs can cancel each other | Keep release concurrency `cancel-in-progress: false` |
