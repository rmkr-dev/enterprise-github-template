...
- **No fake implementations:** no stub functions that pretend to work, no copied badges for CI that is not running, no architecture diagrams of systems that are not in this repo.
...
- This slice has no workflows yet. Do not claim CI exists. When you add workflows, add them as their own reviewable slice and wire docs the same day.
...
- Dependency and code scanning belong in later slices; until then, do not invent a scanner story in the README.
- Treat `docs/security/security.md` as the consumer-facing posture. Update it when defaults change.
...
- Network diagrams are out of scope unless the consumer’s system has a network to document. Do not add decorative diagrams here.
...
3. Confirm you did **not** add Node/npm, sample app code, or workflows unless that was the slice.
4. Confirm commit messages are conventional (`feat:`, `docs:`, `chore:`, …) and read like a person wrote them.
