...
CI runs on GitHub Actions for this template: required-file validation, a small test around that validator, and a shell job (`bash -n` + ShellCheck) — on pull requests, pushes to `main`, and weekly via `validate-scheduled.yml`. CI concurrency cancels superseded runs; release concurrency does not. Pushing a `v*` tag creates a GitHub Release. There is no application build.
