...
GitHub Actions runs those same checks on pull requests, pushes to `main`, and on a weekly schedule (`.github/workflows/validate-scheduled.yml`), plus `bash -n` and ShellCheck on the template scripts. CodeQL analyzes Actions workflow YAML. Pushing a `v*` tag creates a GitHub Release via `.github/workflows/release.yml`. Details: [docs/development/development.md](docs/development/development.md).
...
| [ADR-006](docs/decisions/ADR-006-changelog-backed-github-releases.md) | Why GitHub Release notes embed CHANGELOG sections |
...
| [Upgrading from upstream](docs/operations/upgrading-from-upstream.md) | Adopt later template releases in a derived repo |
...
| [Examples (derived repos)](docs/references/examples.md) | How Java/Python/Go/Rust/.NET/PHP/Ruby/Kotlin/Terraform/Azure-AKS consumers can evolve from this template |
